...
class Model(torch.nn.Module):
    ...
    def _prob_zero(self, param_values: np.ndarray,
        ...
        """
        Estimate P(|00...0⟩) using FakeSherbrooke noise simulation.

        Steps:
        1. Build the ideal circuit with bound parameter values.
        2. Append measure_all() — required for shot-based sampling.
        3. Transpile to Sherbrooke's native gate set (ECR, RZ, SX, X)
            and heavy-hex qubit topology.
        4. Run on the noisy AerSimulator and extract the all-zeros count.
        """
        qc = self._build_circuit(param_values, input_string1, input_string2)
        qc.measure_all()

        qc_t = transpile(qc, noisy_sim, optimization_level=1)
        job  = noisy_sim.run(qc_t, shots=shots)
        counts = job.result().get_counts()

        all_zeros = '0' * self.num_qubit
